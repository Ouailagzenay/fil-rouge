...
if (!class_exists('VC_Extensions_CoverSlider')) {
    class VC_Extensions_CoverSlider{
        function __construct() {
            ...
            "class" => "cm_cq_vc_extension_coverslider",
...
[captionitem]item caption 3[/captionitem]'
      ...
      function cq_vc_coverslider_func($atts, $content=null, $tag) {
          ...
          if(version_compare(cm_VC_VERSION,  "4.4")>= 0){
          ...
          // $content = cm_js_remove_wpautop($content); // fix unclosed/unwanted paragraph tags in $content
          ...
          foreach ($contentarr as $key => $thecontent) {
              ...
              $content_str .= cm_js_remove_wpautop($thecontent);
              $content_str .= '</div>';
          }
          ...
          foreach ($imagesArr as $key => $theimage) {
              ...
              if($isresize=="yes"&&$imagewidth!=""){
                  if(function_exists('cm_resize')){
                      $img = cm_resize($theimage, null, $imagewidth, null);
                      $thumbnail = $img['url'];
